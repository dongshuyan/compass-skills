#!/usr/bin/env python3
"""Validate the released run-history skill upgrader package."""
...
def validate_evals(path: Path, errors: list[str], warnings: list[str]) -> None:
    ...
    if not isinstance(evals, list) or not evals:
        ...
        return
    if len(evals) < 6:
        warnings.append(f"{path}: fewer than 6 eval cases; consider adding more approval and boundary coverage")
...
def validate_semantics(skill_md: Path, errors: list[str]) -> None:
    text = load_text(skill_md)
    required_terms = ("plan_only", "explicit approval", "existing skill")
    for term in required_terms:
        if term not in text:
            errors.append(f"{skill_md}: missing required upgrader contract term {term!r}")
    forbidden = skill_md.parent / "references" / "maintenance-log.md"
    if forbidden.exists():
        errors.append(f"{forbidden}: released upgrader package must not ship an internal maintenance log")
...
def validate_skill(skill_dir: Path) -> tuple[list[str], list[str]]:
    ...
    validate_semantics(skill_md, errors)
...
def main() -> int:
    ...
    parser.add_argument("--skill", required=True, help="Path to the upgrader skill package to validate")
