...
COMPASS 当前公开 6 个本地 skills：4 个运行时协作 skills，加上 2 个 run-history skill 工程 skills。核心协作层仍围绕用户画像、任务图谱、AI 对话续接和需求对齐；新增的 `run-history` 双 skill 则负责把真实运行经验沉淀成新 skill，或安全升级已有 skill。它把长任务、跨 AI 对话协作和多 agent 工作从聊天记录里移到可查、可更新、可续接的本地结构中。
...
| [`run-history-skill-upgrader`](skills/run-history-skill-upgrader/) | 根据真实失败、验证结果和用户反馈，为已有 skill 先产出升级方案，再在明确批准后执行修改。 |

多 skill 仓库建议按需安装；`run-history` 这两个 skill 主要面向设计、发布和维护其他 skills 的场景。
...
**场景 6：根据真实运行反馈升级已有 skill。**
用 `$run-history-skill-upgrader` 读取目标 skill 和本次失败证据，先给出结构化升级方案并停下；只有你明确批准该具体方案后，它才会修改已有 skill 文件。
