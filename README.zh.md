...
COMPASS 当前公开 6 个本地 skills：4 个运行时协作 skills，加上 2 个 run-history skill 工程 skills。核心协作层仍围绕用户画像、任务图谱、AI 对话续接和需求对齐；新增的 `run-history` 双 skill 则负责把真实运行经验沉淀成新 skill，或自动读取真实执行、遇到困难、解决困难、验证结果和用户反馈等 session 信息，推动已有 skill 受控自我进化。它把长任务、跨 AI 对话协作和多 agent 工作从聊天记录里移到可查、可更新、可续接的本地结构中。
